...
const char *versionname="1.0.30";

// #define df_time_upgrade_ms 600000 //10minute*60second*1000ms
// uint32_t u32check_firmware=120000;//2minute*60second*1000ms

#define df_time_upgrade_ms 1800000 //10minute*60second*1000ms
// #define df_time_upgrade_ms 600000
uint32_t u32check_firmware=120000;//2minute*60second*1000ms
...
//   pinMode(led_sign,OUTPUT);
  ...
  Serial.println("-----------------------------------\n");
	...
	// #ifndef DF_MQTT_UP_4G
		...
		clientmqtt.setKeepAlive(3000);//5 minutes
...
void loop() 
{
	...
	// 	// process_upfile();			/*S3*/
		...
		SWITCH_BARCODE();
		xuly_thongtin_mayin();
		///ket thuc phan danh them

		
		// if (system_connection_mode==BYSIM4G)
		// {
		// 	if(kt_connect!=1 && ui8_dem_solan_ketnoimqtt <= 3)
		// 	{
		// 		connect_MQTT_ATCMD();
		// 		ui8_dem_solan_ketnoimqtt++;
		// 		server_modem.busy=0;
		// 		ui8_sub = 1;
		// 	}
		// }
	...
	{		
		...
		if(system_connection_mode==BYSIM4G)
		{
			...
			// {
				...
				// tt=pulic_data(data_get,data_get.length(),channel_);
				// Serial.write(value_moi,strlen(value_moi));
				// int need_upload_opu = res_command(500, "+CMQTTPUB: 0,0");
	...
	if (inforeget.index == 1 && stat==1)
	{
		...
		if (len >0 && ui8_ketnoimqtt_ok==1)
		{
			...
			else
			{
				...
				// ui32_timeout_disconnect_mqtt=millis()+500;
				setup_modem();
				ui32_timeout_guilailannua = millis() + 2000;
				if(ui8_demsolanguisai_chunk>3)
				{
					ESP.restart();
					ui8_demsolanguisai_chunk=0;
...
void load_setting_default(String file, str_system_info *regs)
{
	...
	json1["wifi"] = "1";
	json1["modem"] = "2";
	json1["domain"] = "dcs-mqtt.altacloud.biz";
	json1["username"] = "dcsprinter";
	json1["password"] = "AmDcsprt0604";
	...
	sprintf(regs->name_domain,"%s","dcs-mqtt.altacloud.biz");
	sprintf(regs->username_mqtt,"%s","dcsprinter");
	sprintf(regs->password_mqtt,"%s","AmDcsprt0604");
...
int FirmwareVersionCheck(void) 
{
  ...
  if (httpCode == HTTP_CODE_OK) // if version received
  {
	...
	*/
	...
	if(device_status.vs_printer[2] == '4')
	{
		strlcpy(chuoi,                  // <- destination
	...
	if(device_status.vs_printer[2] == '5')
	{
		strlcpy(chuoi,                  // <- destination
	...
	if(device_status.vs_printer[2] == '6')
	{
		strlcpy(chuoi,                  // <- destination
...
void checkfirmware()
{
	...
	/* =1: update server, =2; update printer else: undefined */
		...
		else if (luachon == 3) 
		{
			...
			ui8_ketnoi_github_wifi_barcode=1;
			// Serial.println("connect update wifi");
			// Serial.println("save file bin for STM32");
			// connected_server(sclient, test_root_ca, "raw.githubusercontent.com", 443, "Danh555/file_code/main/STM32F107RCT6_new.bin");
			// ui8_update_fw_wifi=1;
...
void connect_mqtt()
{
	...
	if (clientmqtt.connect(mynamedevice, systeminfo.username_mqtt , systeminfo.password_mqtt))
	{
		...
		sprintf(au_topic_pub, "%s/%s/QUERY",inforeget_setting.name_topic, mynamedevice);
		// send_at("ATE1");
		// init_MQTT();
		// connect_MQTT_ATCMD();
		// send_at("AT+CMQTTCFG=\"checkUTF8\",0,0");
	...
	// if (millis() - lastMillis > df_time_send_opu) {
		...
		*/
		...
		else
		{
			...
			u32_time_resend_opu = millis() + 35000;
...
void upload_code_STM32_barcode()
{
	...
	if(ui8_ketnoi_github_wifi_barcode==1)
	{	
		if(dem_thatbai<=5)
		{
			if(ui8_ketnoithatbai == 1 || (ui8_ketnoithatbai==0 && ui8_ketnoithanhcong == 0))
			...
			{
				ui8_update_fw_wifi=1;
				ui8_update_fw_barcode=1;
				ui8_ketnoi_github_wifi_barcode = 0;
			}
		}
	...
	else if(ui8_update_fw_wifi==1 && ui8_update_fw_barcode==1)
	{
		Serial.println("dang luu file cho BARCODE wifi");
		save_file(sclient,file_update_wifi_barcode);
		Serial1.end();
		Serial2.end();
		device_status.vs_barcode[0] ='N';/* xóa version cũ, để cập nhật tên version mới */
		device_status.vs_barcode[1] ='O';/* xóa version cũ, để cập nhật tên version mới */
...
void connect_MQTT_ATCMD()
{
	...
	kt_connect = sendATCommand("AT+CMQTTCONNECT=0,"+(String) (domain_name)+",64800,0,"+(String) (username)+","+(String) (password),500);
...
void sleep_box()
{
	if(ui32_timeout_checksleepbox > millis())
	{
		// Serial.println("chua du thoi gian");
		return;
	}
...
void check_SIM(long waktu)
{
	...
	// test2=cmd.substring(a,b+1);

	// // Serial_ARDUIRNO.println("cmd sau khi check la: ");
	// Serial_ARDUIRNO.println(test);
	// if(test=="OK")
	// {
	// ketqua=1;
	// }
