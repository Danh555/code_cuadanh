...
void uploadLargeFile(const char *ten,int check_buffer) {
  // Mở file
  ...
  int j = 'a';
	...
	*/
	...
	Serial.printf("Ghi vào file: %s\n", ten);

	File file = SPIFFS.open(ten, FILE_WRITE);
	if (!file) {
  ...
  File f = SPIFFS.open(ten, "r");
  if (!f) {
    Serial.println("Không mở được file");
    return;
  }
  size_t contentLength = f.size();

  // Tạo boundary
  String boundary = "----ESP32Boundary";
  String header = String("--") + boundary + "\r\n"
                + "Content-Disposition: form-data; name=\"fileToUpload\"; filename=\"data294kb_4G.txt\"\r\n"
...
void check_serial_debug()
{
  ...
  if (Serial.available()) {
	...
	{
		Serial.println("test up file https");
		...
		uploadLargeFile_ppp(SPIFFS,ten,10000);
	}
...
void test_upload_data_SPIFFS()
{
	...
	if (data)
	{
		...
		Serial.println("data tao moi nhap vao");
...
bool ensurePPPConnected(int retry = 3) {
  for (int i = 0; i < retry; i++) {
    if (PPP.connected()) return true;
    Serial.println("[PPP] Mất kết nối, thử reconnect...");

    // Nếu có PPP.end() thì dùng, nếu không thì bỏ qua dòng này
    PPP.end();
    // Nếu cần, reset module bằng chân RST ở đây

    PPP.begin(PPP_MODEM_MODEL, 1, 460800);
    delay(2000);

    if (PPP.connected()) {
      Serial.println("[PPP] Đã kết nối lại thành công!");
      return true;
    }
    delay(2000);
  }
  Serial.println("[PPP] Lỗi: Không thể kết nối lại sau nhiều lần thử!");
  return false;
}
...
void setup_ppp()
{
	...
	// delay(1000);
	PPP.begin(PPP_MODEM_MODEL,1,115200);



	Serial.print("Manufacturer: ");
	String manu = PPP.cmd("AT+CGMI", 10000);
	Serial.print("[");
	Serial.print(manu);
	Serial.println("]");
	Serial.print("Model: ");
...
void uploadLargeFile_ppp(fs::FS &fs,const char *ten,int check_buffer) 
{
  ...
  String boundary = "----ESP32Boundary";
  String header = String("--") + boundary + "\r\n"
                + "Content-Disposition: form-data; name=\"fileToUpload\"; filename=\"data294kb_4G_PPP.txt\"\r\n"
                + "Content-Type: application/octet-stream\r\n\r\n";
  String footer = "\r\n--" + boundary + "--\r\n";
