...
void loop() 
{
	...
	// 	// process_upfile();			/*S3*/
		...
		// connect_MQTT();
		// connect();
		//  if (!sendPing(mynamedevice)) {
		// 	Serial.println("MQTT connection lost, reconnecting...");
		// 	connectMQTT();  // Gọi hàm để kết nối lại
		// }
		// reconnect_mqtt(mqtt_gsm,mynamedevice);
		#if (DF_USING_BARCODE)
		SWITCH_BARCODE();
		#endif
		xuly_thongtin_mayin();
	...
	if(system_running_mode ==0)
	{
		...
		if(u32check_firmware>millis() || ui8_busy_rec_printer == 0|| ui8_dangupdate_mqtt==1)
		{
		...
		else
		{
			...
			if(ui16_counterfail<=1)
			{
				...
				if(system_connection_mode == BYSIM4G)
				{
					...
					ui32_time_allow_upfile = millis() + 5000;
					// ui8_uploadlai=1;
				}
			...
			else if(ui16_counterfail > 1 && ui16_counterfail<=5)
			{
				ui8_dangupdate_mqtt=0;
				// ui16_counterfail=0;

				ui32_time_allow_upfile = millis() + 10000;
				//reset
				Serial.println("-------------restart here------");
				// ESP.restart();
				if(system_connection_mode == BYSIM4G)
...
void connect_mqtt()
{
	...
	// sprintf(test, "\"client test%s\"", myname_device);
		...
		// if(ui8_check_disconnect_mqtt==0)
		// {
			// String datastatus = "online";
			// Serial.println("Online");
			// pulic_data(datastatus,sizeof(datastatus),au_topic_pub);
			String datastatus_onl = "online";
			ui8_demsolan_ketnoi_mqtt=0;
			...
			// clientmqtt.publish(au_topic_pub, datastatus, 1, 2);
			// pulic_data_TEST(datastatus,strlen(datastatus),au_topic_pub);
			// subscribeTopics(au_topic_resetserver);
			int ketqua = pulic_data_onl(datastatus_onl,datastatus_onl.length(),au_topic_pub);
			// send_at("ATE1");
			// init_MQTT();
			// connect_MQTT_ATCMD();
			// send_at("AT+CMQTTCFG=\"checkUTF8\",0,0");
...
{			
	//kiểm tra
	if(ui8_rs_server==1||ui8_rs_printer==1||ui8_rs_barcode==1||ui8_upfw_server==1||ui8_upfw_barcode==1||ui8_luafw_printer==1||ui8_kt_sniff==1||ui8_kt_chunk==1)
	{
