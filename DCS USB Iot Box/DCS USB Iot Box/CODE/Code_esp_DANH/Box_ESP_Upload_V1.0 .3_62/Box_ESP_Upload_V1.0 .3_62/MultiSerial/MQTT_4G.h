...
int sendATCommand(String command, const int timeout) 
{
  ...
  {
    if((data_2=="OK" && test2 =="CMQTTPUB: 0,0"))
    {
  ...
  {
    if(test=="OK" || (data_2=="OK" && test2 =="+CMQTTCONNECT: 0,0"))
    {
      ketqua=1;
    }
...
void init_MQTT()
{
  ...
  // wRespon(100);
  // delay(100);
	// send_at("AT+CMQTTACCQ=0,\"client test0\"");
  // wRespon(100);
  // delay(100);

 // phan DANH DANG SU DUNG//
  // wRespon(100);
  // send_at("AT+CSQ");
  // wRespon(100);
	// send_at("AT+CMQTTSTART");
  ...
  // wRespon(100);
  // KET THUC phan DANH DANG SU DUNG//


  sendATCommand("AT+CSQ",100);
  sendATCommand("AT+CMQTTSTART",100);
  sendATCommand("AT+CMQTTACCQ=0,\"client test0\"",100);
  sendATCommand("AT+CMQTTCFG=\"checkUTF8\",0,0",100);
}
...
void disconnect_MQTT()
{
  ...
  sendATCommand("AT+CMQTTDISC=0,180",50);
  sendATCommand("AT+CMQTTREL=0",50);
  sendATCommand("AT+CMQTTSTOP",50);
}
