...
void
AWSClient4::createRequestHeaders_kichthuoc(char* signature, int kichthuoc)
{
  headers[0] = 0;
  sprintf(headers, "%sContent-Type: %s\r\n", headers, contentType);
  sprintf(headers, "%sContent-Length: %d\r\n", headers,
...
char*
AWSClient4::createRequest_kichthuoc(MinimalString &reqPayload, int kichthuoc)
{
    /* Check that all values have been initialized. */
    if (awsRegion == 0 || awsEndpoint == 0 || awsSecKey == 0 || awsKeyID == 0
            || httpClient == 0 || dateTimeProvider == 0)
    ...
    SHA256* sha256 = new SHA256();
    // payloadHash = (*sha256)(reqPayload.getCStr(), reqPayload.length());
    payloadHash = (*sha256)(reqPayload.getCStr(), kichthuoc);
    delete sha256;
    ...
    // create the headers
    // createRequestHeaders(signature);
    createRequestHeaders_kichthuoc(signature, kichthuoc);
    delete signature;
...
const char*
AWSClient4::sendData_kichthuoc (const char* data,const char* payload, int kichthuoc)
{
    // char* server = createHost();
    int port = httpS ? 443 : 80;
    ...
    // Serial.println("path");
    // int port = 443;
    const char* response = httpClient->send_kichthuoc(data, awsDomain, port, payload, kichthuoc);
    // printf("line 331");
    // delete[] server;
    return response;
}
