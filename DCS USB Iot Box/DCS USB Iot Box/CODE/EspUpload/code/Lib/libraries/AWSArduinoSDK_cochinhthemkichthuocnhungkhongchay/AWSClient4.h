...
class AWSClient4 {
    ...
    void createRequestHeaders_kichthuoc(char* signature, int kichthuoc);
...
protected:
    ...
    char* createRequest_kichthuoc(MinimalString &payload, int kichthuoc);
    void createCanonicalRequest();
    void createCanonicalHeaders();

    char *createRequestUploadFile(MinimalString &reqPayload);

    /* Sends http data. Returns http response, or null on error. */
    const char* sendData(const char* data);
    const char* sendData_kichthuoc (const char* data,const char* payload, int kichthuoc);
