...
const char*
EspHttpClient::send_kichthuoc(const char* request, const char* serverUrl, int port, const char* payload, int kichthuoc)
{
    //port = 443;
#if !defined ARDUINO_ARCH_SAM && !defined ARDUINO_ARCH_SAMD
    WiFiClientSecure sclient;
    ...
    WiFiSSLClient sclient;
#endif
    Serial.println("in send_kichthuoc function");
    Serial.print("serverUrl = ");
    Serial.println(serverUrl);
    Serial.print("port = ");
    Serial.println(port);
    ...
    if (sclient.connect(serverUrl, port)) {
        ...
        // Send the request
        sclient.print(request);
        // sclient.print(payload);
        // sclient.print("\r\n\r\n");
        //send payload

        // sclient.write((const uint8_t *)payload,kichthuoc);
        // for(int i =0; i < kichthuoc ; i++)
        // {
        //     sclient.write(*(payload+i));
        // }

        // sclient.print("\r\n\r\n");
        Serial.println("end send request");
