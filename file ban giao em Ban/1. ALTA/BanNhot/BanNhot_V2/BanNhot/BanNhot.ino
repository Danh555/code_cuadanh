...
void guilenhlenPC()
{
  if (UART_PC.timeSend + THOIGIANCHOGUILENH <= millis())
  {
    ...
    // GUILENH(ui2_respond_reset, 0x10, 0x00, respondReset(function_code));
  }
}
...
void respondIncorrectCommand(uint8_t function_code, uint8_t status)
{
  uint8_t bytecount = 2;
  uint8_t datasize = bytecount + 4;
  uint8_t data[datasize + 1] = {datasize, 0xFE, ui8_data_receive[2], ui8_data_receive[3], bytecount, function_code, status};
  memcpy(ui8_data_send, data, datasize + 1);
  UART_PC.send(ui8_data_send);
}
...
// void respondReset(uint8_t function_code)
// {
//   uint8_t bytecount = 1;
//   uint8_t datasize = bytecount + 4;
//   uint8_t data[datasize + 1] = {datasize, function_code, ui8_data_receive[2], ui8_data_receive[3], 0x02, 0x00};
//   memcpy(ui8_data_send, data, datasize + 1);
//   UART_PC.send(ui8_data_send);
//   ui32_timesend = millis();
// }
...
//   {
//     if (SENSOR[i].change == 1)
//     {
//       sendState(SENSOR[i].Pin, SENSOR[i].state);
//       SENSOR[i].change = 0;
//     }
//   }
// }
...
void processSerial()
{
  ...
  if (UART_PC.dataReceived == true)
  {
    ...
    case 0x04:
      ...
      if (ui2_respond_function_notExist == 0)
...
void setLEDPixel()
{
  ...
  setLEDPixel(&ThungNhot1);
  setLEDPixel(&ThungNhot2);
  setLEDPixel(&ThungNhotThai);
}
void setLEDPixel(ThungNhot *thungnhot)
{
  ...
  if ((*thungnhot).trangthai_LED != (*thungnhot).trangthai_thung)
  {
    ...
    if ((*thungnhot).trangthai_LED == BINHTHUONG)
      setOneColorAllStrip(thungnhot, 255, 0, 0); // Nếu vòi bình thường thì đỏ
...
// void reset(ThungNhot *thungnhot)
// {
//   (*thungnhot).trangthai_thung = BINHTHUONG;
//   (*thungnhot).trangthai_thung_nhantuPC = NO_STATE;
//   (*thungnhot).trangthai_voi = NGUNGBAN;
//   (*thungnhot).trangthai_voi_nhantuPC = NO_STATE;
//   (*thungnhot).trangthai_LED = NO_STATE;
//   (*thungnhot).time_hieuung = 0;
//   (*thungnhot).state_hieuung = 0;
// }
