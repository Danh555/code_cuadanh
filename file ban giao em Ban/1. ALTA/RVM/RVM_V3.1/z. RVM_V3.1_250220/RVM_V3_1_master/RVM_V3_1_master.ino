...
void send_oder_to_slave()
{
      ...
      (millis() - RS485.timeSend > 10))      // Giãn cách truyền tránh bị gộp lệnh
  {
    ...
    if (ui8_erase_error_slave != 0x00) // Reset toàn bộ slave
    {
      reset_slave_RS485(ui8_erase_error_slave);
      ui8_erase_error_slave--;
    }
...
void reset_slave_RS485(uint8_t slaveID)
{
  uint8_t bytecount = 1;
  uint8_t datasize = bytecount + 4;
  raiseOrderCommand(self_order_command);
...
void serialDebugProcess()
{
  ...
  //     if (inputString == "cua_fw")
  //   {
  //      MotorBan.YeuCau = TrangThaiMotorBangTai::CHAYVAO;
  //     SerialDebug.println("bangtai_fw");
  //   }

  //   if (inputString == "bangtai_fw")
  //   {
  //      MotorBangTai.YeuCau = TrangThaiMotorBangTai::CHAYVAO;
  //     SerialDebug.println("bangtai_fw");
  //   }
