...
void send_state_to_PC(uint8_t function_code){
    ...
    *(data_send+4) = 0x01;
	...
	{
		*(data_send+4) = 0x02;
		if(thungrac_chai.ui8_phanhoitrangthai_thungrac == 1)
		{
			if(ui8_trangthaithungracchai == TrangThaiThungRac::DAY)
			{
				*(data_send+5) = 0x01;
				*(data_send+6) = 0xFF;
				Serial_Debug.println("THUNG RAC CHAI DAY");
			}
			else{
				*(data_send+5) = 0x01;
				*(data_send+6) = 0x00;
				Serial_Debug.println("THUNG RAC CHAI BINH THUONG");
			} 
				
			ui8_phanhoi_trangthai_thungracchai = 0;
		}
		else if(thungrac_lon.ui8_phanhoitrangthai_thungrac == 1)
		{
			if(ui8_trangthaithungraclon == TrangThaiThungRac::DAY){
				*(data_send+5) = 0x02;
				*(data_send+6) = 0xFF;
				Serial_Debug.println("THUNG RAC LON DAY");
			}
				
			else{
				*(data_send+5) = 0x02;
				*(data_send+6) = 0x00;
				Serial_Debug.println("THUNG RAC LON BINH THUONG");
			}
				
			ui8_phanhoi_trangthai_thungraclon = 0;
		}

		else if(thungrac_khac.ui8_phanhoitrangthai_thungrac == 1)
		{
			if(ui8_trangthaithungrackhac == TrangThaiThungRac::DAY){
				*(data_send+5) = 0x03;
				*(data_send+6) = 0xFF;
				Serial_Debug.println("THUNG RAC KHAC DAY");
			}
				
			else{
				*(data_send+5) = 0x03;
				*(data_send+6) = 0x00;
				Serial_Debug.println("THUNG RAC KHAC BINH THUONG");
			}
	...
	else if(function_code == 0x06)
	{
		*(data_send+4) = 0x03;
		...
		{
			if(ui8_trangthai_nhaqua==THANHCONG)
			{
				*(data_send+5) = 0x01;
				*(data_send+6) = ui8_vitrikhay_sp;
				*(data_send+7) = ui8_vitri_sp;
				Serial_Debug.printf("nha qua thanh cong tai vi tri khay %d va vi tri o %d\n", ui8_vitrikhay_sp, ui8_vitri_sp);
			}

			if(ui8_trangthai_nhaqua==THATBAI)
			{
				*(data_send+5) = 0x02;
				*(data_send+6) = ui8_vitrikhay_sp;
				*(data_send+7) = ui8_vitri_sp;
				Serial_Debug.printf("nha qua that bai tai vi tri khay %d va vi tri o %d\n", ui8_vitrikhay_sp, ui8_vitri_sp);
			}

			if(ui8_trangthai_nhaqua==BINHTHUONG)
			{
				*(data_send+5) = 0x00;
				*(data_send+6) = ui8_vitrikhay_sp;
				*(data_send+7) = ui8_vitri_sp;
				Serial_Debug.printf("da lay qua ra");
			}
		...
		*(data_send+4) = 0x02;
		if(ui8_phanhoi_trangthai_resetthungrac_chai == 1)
		{
			...
			// {
				*(data_send+5) = 0x01;
				*(data_send+6) = 0x00;
				Serial_Debug.println("DA RESET TRANG THAI THUNG RAC CHAI");	
			// }
	...
	else if(ui8_phanhoi_trangthai_resetthungrac_lon == 1)
	{
		...
		*(data_send+5) = 0x02;
		*(data_send+6) = 0x00;	
		Serial_Debug.println("DA RESET TRANG THAI THUNG RAC LON");
		// }
	...
	else if(ui8_phanhoi_trangthai_resetthungrac_khac == 1)
	{
		...
		*(data_send+5) = 0x03;
		*(data_send+6) = 0x00;	
		Serial_Debug.println("DA RESET TRANG THAI THUNG RAC khac");
		// }
	...
	// // 			*(data_send+5) = 0x01;
	// // 			*(data_send+6) = 0xFF;	
	// // 			Serial_Debug.println("DONG CO NHA VAT QUAY DUOC MOT VONG");
	// // 		}
	...
	else if(function_code == 0xFA)
	{
		*(data_send+4) = 0x02;
		...
		if(ui8_phanhoi_mocuathungrac == 1)
		{
			if(ui8_trangthaicuathungrac == 1)
			{
				*(data_send+5) = 0x02;
				*(data_send+6) = 0xFF;
			}

			else if(ui8_trangthaicuathungrac == 0)
			{
				*(data_send+5) = 0x02;
				*(data_send+6) = 0x00;	
			}
			
			ui8_phanhoi_mocuathungrac = 0;
		}
		bytecount = 7;
	}
	...
	{
		if(ui8_trangthaivat == TrangThaiVat::COVAT || motor_ep.ui8_kiemtra_trangthailoi == TrangthaiLoi::LOI || motor_nghien.ui8_kiemtra_trangthailoi == TrangthaiLoi::LOI || 
		motor_nangha.ui8_kiemtra_trangthailoi_motorDC == TrangthaiLoi::LOI || motor_phanloai.ui8_kiemtra_trangthailoi == TrangthaiLoi::LOI||ui8_phathien_lon==1) 
		{
			*(data_send+5) = 0xFF;
		}
		else 
    ...
    *(data_send+4) = 0x02;
	...
	if(thungrac == THUNGRACCHAI){
		*(data_send+5) = 0x01;
		...
		Serial_Debug.printf("PHAN TRAM TRONG THUNG RAC CHAI = %d\n", phantram);
	}

	else if(thungrac == THUNGRACKHAC)
	{
		*(data_send+5) = 0x03;
		phantram = (uint8_t)((uint32_t)(ui16_vat_trongthungrackhac * 100) / (uint16_t)SOCHAITOIDA);
		Serial_Debug.printf("PHAN TRAM TRONG THUNG RAC khac = %d\n", phantram);
	}

	else{
		*(data_send+5) = 0x02;
		phantram = (uint8_t)((uint32_t)(ui16_solon_trongthungrac * 100) / (uint16_t)SOCHAITOIDA);
		Serial_Debug.printf("PHAN TRAM TRONG THUNG RAC LON = %d\n", phantram);
	}
