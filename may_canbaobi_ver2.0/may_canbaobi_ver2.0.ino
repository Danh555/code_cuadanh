...
void setup() {
	...
	setting_Button(&btn,A3,1000); // Khai báo nút nhấn
    scan_i2c();
...
void button_chucnang()
{
	...
	if(ui8_modesetting==1 && ui8_nut1_bam==0)
	{
		int reading = digitalRead(buttonPin);
	...
	if(ui8_modesetting==1 && ui8_nut1_bam==1)
	{
		int reading = digitalRead(buttonPin);
...
void nutnhan()
{
	if(ui8_bamnut==0 && ui32_tg_bamnut<=millis()) 
	{
		// Serial_debug.println("khong vao che do settings");
		return;
	}
	...
	if(ui8_modesetting==0)
	{	
		// Serial_debug.println("vao che do settings");
		// trangthai_nutnhan=0;
		// trangthai_nutnhan=readButton(&btn,1000);
		if(readButton(&btn,1000)==1)
		{
			weights=0;
			weightsTemp=0;
			Serial_debug.println("Nút nhấn bấm vào chế độ settings");
	...
	{
		// trangthai_nutnhan=0;
		// trangthai_nutnhan=readButton(&btn,50);
		if(readButton(&btn,50)==1)
		{
			// Nút được nhấn
			Serial_debug.println("vao che do settings doc nut chinh");
			ui8_solan_bamnut++;
			if(ui8_solan_bamnut>3)
			{
				ui8_solan_bamnut=0;
