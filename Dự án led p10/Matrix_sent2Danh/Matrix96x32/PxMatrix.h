...
#define _BV(x) (1 << (x))
  ...
  // #define SPI_BYTE(x) SPI.write(x) //mrcong
...
enum scan_patterns {LINE, ZIGZAG,ZZAGG, ZAGGIZ, WZAGZIG, VZAG, ZAGZIG, WZAGZIG2, ZZIAGG, ZAGOUTDOOR};
...
class PxMATRIX : public Adafruit_GFX {
 ...
 // the display buffer for the LED matrix
  ...
  uint8_t _CLK_PIN=32;
	...
	//PORT2
	uint8_t _P2R1_PIN=2;
	uint8_t _P2G1_PIN=19;
	uint8_t _P2B1_PIN=15;
	uint8_t _P2R2_PIN=4;
	uint8_t _P2G2_PIN=21;
	uint8_t _P2B2_PIN=26;
	//PORT1
	uint8_t _P1R1_PIN=13;
	uint8_t _P1G1_PIN=5;
	uint8_t _P1B1_PIN=14;
	uint8_t _P1R2_PIN=27;
	uint8_t _P1G2_PIN=0;
	uint8_t _P1B2_PIN=12;

#else
	//PORT2
	uint8_t _P2R1_PIN=13;
	uint8_t _P2G1_PIN=19;
	uint8_t _P2B1_PIN=14;
	uint8_t _P2R2_PIN=27;
	uint8_t _P2G2_PIN=21;
	uint8_t _P2B2_PIN=12;
	//PORT1
	uint8_t _P1R1_PIN=2;
	uint8_t _P1G1_PIN=5;//rx2=16
	uint8_t _P1B1_PIN=15;
	uint8_t _P1R2_PIN=4;
	uint8_t _P1G2_PIN=0;//tx2=17
	uint8_t _P1B2_PIN=26;
...
//PORT1
	uint8_t _P1R1_PIN=13;
  uint8_t _P1G1_PIN=19;
  uint8_t _P1B1_PIN=14;
  uint8_t _P1R2_PIN=27;
  uint8_t _P1G2_PIN=21;
  uint8_t _P1B2_PIN=12;
//PORT2
	uint8_t _P2R1_PIN=2;
  uint8_t _P2G1_PIN=16;
  uint8_t _P2B1_PIN=15;
  uint8_t _P2R2_PIN=4;
  uint8_t _P2G2_PIN=17;
  uint8_t _P2B2_PIN=26;
...
*/
  ...
  uint8_t u8state;//mrCong mới thêm
...
inline void SPI_BYTE(uint8_t reg_value);
inline void SPI_6BYTE(uint8_t r1_value,uint8_t g1_value,uint8_t b1_value,uint8_t r2_value,uint8_t g2_value,uint8_t b2_value);
inline void SPI_12BYTE(uint8_t r22_value,uint8_t r21_value,uint8_t r12_value,uint8_t r11_value,uint8_t g22_value,uint8_t g21_value,uint8_t g12_value,uint8_t g11_value,uint8_t b22_value,uint8_t b21_value,uint8_t b12_value,uint8_t b11_value);
inline void SPI_TRANSFER(uint8_t* x,uint16_t y);
};
...
//mrcong add
inline void PxMATRIX::SPI_BYTE(uint8_t reg_value)
{
	uint8_t mask;
	for (uint8_t i=0; i < 8; i++)
    {
		mask=0x80>>i;
		digitalWrite(_P1R1_PIN,reg_value & mask);
		digitalWrite(_P1G2_PIN,reg_value & mask);
      // delay(1);
      digitalWrite(_CLK_PIN,LOW); // CLK HIGH
      // delay(1);
      digitalWrite(_CLK_PIN,HIGH); // CLK LOW
      // delay(1);
    }
}

//4R1->4R2->4G1->4G2->4B1->4B2
//mrcong add

inline void PxMATRIX::SPI_6BYTE(uint8_t r22_value,uint8_t r21_value,uint8_t g22_value,uint8_t g21_value,uint8_t b22_value,uint8_t b21_value)
{
	uint8_t mask;
	for (uint8_t i=0; i < 8; i++)
    {
		mask=0x80>>i;
		// digitalWrite(_P1R1_PIN,r1_value & mask);
		// digitalWrite(_P1G1_PIN,g1_value & mask);
		// digitalWrite(_P1B1_PIN,b1_value & mask);
		// digitalWrite(_P1R2_PIN,r2_value & mask);
		// digitalWrite(_P1G2_PIN,g2_value & mask);
		// digitalWrite(_P1B2_PIN,b2_value & mask);
    digitalWrite(_P2R1_PIN,r21_value & mask);
		digitalWrite(_P2G1_PIN,g21_value & mask);
		digitalWrite(_P2B1_PIN,b21_value & mask);
		digitalWrite(_P2R2_PIN,r22_value & mask);
		digitalWrite(_P2G2_PIN,g22_value & mask);
		digitalWrite(_P2B2_PIN,b22_value & mask);
      // delay(1);
      digitalWrite(_CLK_PIN,LOW); // CLK HIGH
      // delay(1);
      digitalWrite(_CLK_PIN,HIGH); // CLK LOW
...
inline void PxMATRIX::SPI_12BYTE(uint8_t r22_value,uint8_t r21_value,uint8_t r12_value,uint8_t r11_value,uint8_t g22_value,uint8_t g21_value,uint8_t g12_value,uint8_t g11_value,uint8_t b22_value,uint8_t b21_value,uint8_t b12_value,uint8_t b11_value)
{
	uint8_t mask;
	for (uint8_t i=0; i < 8; i++)
    {
		mask=0x80>>i;
		//port1
		digitalWrite(_P1R1_PIN,r11_value & mask);
		digitalWrite(_P1G1_PIN,g11_value & mask);
		digitalWrite(_P1B1_PIN,b11_value & mask);
		digitalWrite(_P1R2_PIN,r12_value & mask);
		digitalWrite(_P1G2_PIN,g12_value & mask);
		digitalWrite(_P1B2_PIN,b12_value & mask);
...
//mrcong add
// inline void PxMATRIX::SPI_TRANSFER(uint8_t *x,uint16_t y)
// {
// 	uint16_t onecolor=y/12;	
// 	for (uint16_t i=0; i < y/12; i++)
// 	{		
// 		// SPI_6BYTE(x[onecolor*5+i],0,0,0,0,0);	
// 		// SPI_12BYTE(x[i],x[onecolor+i],x[onecolor*2+i],x[onecolor*3+i],x[onecolor*4+i],x[onecolor*5+i],x[onecolor*6+i],x[onecolor*7+i],x[onecolor*8+i],x[onecolor*9+i],x[onecolor*10+i],x[onecolor*11+i]);
// 	}
// }
...
inline void PxMATRIX::SPI_TRANSFER(uint8_t *x,uint16_t y)
{
	uint16_t onecolor=y/6;	
	for (uint16_t i=0; i < y/6; i++)
	{		
		//SPI_6BYTE(x[onecolor*5+i],0,0,0,0,0);	
		SPI_6BYTE(x[i],x[onecolor+i],x[onecolor*2+i],x[onecolor*3+i],x[onecolor*4+i],x[onecolor*5+i]);		
	}
}
 ...
 * Get buffer state
 *
 * @return = 0 normal, = 1 over buffer
 * @ingroup buffer
 */
inline uint8_t PxMATRIX::getState()
{
    return u8state;
}
...
inline void PxMATRIX::setMuxPattern(mux_patterns mux_pattern)
{
  _mux_pattern=mux_pattern;
    ...
    pinMode(_D_PIN, OUTPUT);//DANH MOI THEM
...
inline void PxMATRIX::fillMatrixBuffer(int16_t x, int16_t y, uint8_t r, uint8_t g, uint8_t b,bool selected_buffer)
{
  ...
  else if (_scan_pattern!=LINE && _scan_pattern!=WZAGZIG && _scan_pattern!=VZAG && _scan_pattern!=WZAGZIG2)
  {
    // Precomputed row offset values
    base_offset=_row_offset[y]-(x/8)*2;
    uint8_t row_sector=0;
  ...
  //
  // For example  [2L|2H] [0L|0H] for ZAGOUTDOOR<=ZIGZAG
  //                 |   \   |   
  //              [3H|3L] [1H|1L] 
  //
  //
  ...
  // Serial.println("total_offset_r=" + String(total_offset_r));
...
void PxMATRIX::begin(uint8_t row_pattern) {
  ...
  digitalWrite(_OE_PIN, HIGH);
	...
	pinMode(_P1R1_PIN, OUTPUT);
	pinMode(_P1G1_PIN, OUTPUT);
	pinMode(_P1B1_PIN, OUTPUT);
	pinMode(_P1R2_PIN, OUTPUT);
	pinMode(_P1G2_PIN, OUTPUT);
	pinMode(_P1B2_PIN, OUTPUT);
	pinMode(_P2R1_PIN, OUTPUT);
	pinMode(_P2G1_PIN, OUTPUT);
	pinMode(_P2B1_PIN, OUTPUT);
	pinMode(_P2R2_PIN, OUTPUT);
	pinMode(_P2G2_PIN, OUTPUT);
	pinMode(_P2B2_PIN, OUTPUT);

	digitalWrite(_P1R1_PIN, LOW);
	digitalWrite(_P1G1_PIN, LOW);
	digitalWrite(_P1B1_PIN, LOW);
	digitalWrite(_P1R2_PIN, LOW);
	digitalWrite(_P1G2_PIN, LOW);
	digitalWrite(_P1B2_PIN, LOW);
	digitalWrite(_P2R1_PIN, LOW);
	digitalWrite(_P2G1_PIN, LOW);
	digitalWrite(_P2B1_PIN, LOW);
	digitalWrite(_P2R2_PIN, LOW);
	digitalWrite(_P2G2_PIN, LOW);
	digitalWrite(_P2B2_PIN, LOW);
