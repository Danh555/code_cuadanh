...
// SSLClient myclient(base_client, TAs, (size_t)TAs_NUM, rand_pin);
...
void UDHttp::sendChunk(SSLClient& client, uint8_t *buf, int len){
    int idx = 0;
    size_t result;
    while(len > 0){
        if(len < CHUNK_SIZE){
...
int UDHttp::upload(SSLClient& client,char *uploadUrlHandler, char *fileName, int sizeOfFile, DataCb dataCb, ProgressCb progressCb, DataCb responseCb){
...
int UDHttp::download(SSLClient& client, char *downloadUrl, char *downloadFile, DataCb dataCb, ProgressCb progressCb){
    char buf[HEADER_SIZE];
    char host[HOST_LEN];
    int port = 443;
    // WiFiClient client;
